...
function findAllBikes() {
    return prisma_1.default.bike.findMany({ include: { category: true, owner: { select: { id: true, firstName: true, lastName: true, emails: { where: { isPrimary: true }, select: { email: true } } } } } });
}
function findBikeById(id) {
    return prisma_1.default.bike.findUnique({ where: { id }, include: { category: true, owner: { select: { id: true, firstName: true, lastName: true, emails: { where: { isPrimary: true }, select: { email: true } } } } } });
}
